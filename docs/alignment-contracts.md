...
| `dm-overview-send-trend` | 14-month line chart (First class / Standard / Total) | PCM `/order` grouped by orderDate month, minus test + canceled | PCM `/order` (same) | monthly totals must match PCM pagination sum; per-month drift > 5% on any month → `red` | — | `refresh-cache` |
