...
| `dm-overview-send-trend` | Stacked bar chart, monthly volume: past months show full-month totals (end-of-month); the current month shows month-to-date through today; split First class / Standard. Current-month bar is rendered at reduced opacity to signal it's partial. X-axis: "MMM YYYY" (e.g. "Apr 2026"); tooltip: past months "April 2026 (full month)", current month "April 1–22, 2026 (month-to-date)". Rolls over automatically at month-end. | `computeSendTrend.lifetimeTotal` (lifetime client-order count, unfiltered — independent of the rendering cutoff) | `headline.lifetimePieces.pcm` (PCM `/order` paginated total, clients only) | the hub's `lifetimeTotal` must equal `headline.lifetimePieces.pcm` within 1% — both derive from the same PCM pagination, so any drift means a grouping/filter regression in the hub. | red > 1% | `refresh-cache` |
