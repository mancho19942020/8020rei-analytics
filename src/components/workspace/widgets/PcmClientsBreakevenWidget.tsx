/**
 * Break-even Clients Widget (margin 0-5%)
 *
 * AxisTable showing clients near zero margin, sorted by margin % descending.
 * Data source: dm_client_funnel filtered to marginPercent 0-5%
 */
...
const columns: Column[] = [
  ...
  { field: 'margin', header: 'Margin', sortable: true, type: 'currency', align: 'center', width: 100 },
  {
    ...
    render: (value: CellValue) => (
      <AxisTag color="alert" size="sm">{Number(value || 0).toFixed(1)}%</AxisTag>
    ),
  },
];

export function PcmClientsBreakevenWidget({ data }: Props) {
  const tableData = useMemo(() => {
    if (!data?.clients?.length) return [];
    return data.clients
      .filter(c => c.marginPercent >= 0 && c.marginPercent <= 5)
  ...
  if (tableData.length === 0) {
    return (
      <div className="flex items-center justify-center h-full text-sm" style={{ color: 'var(--text-tertiary)' }}>
        No clients in the 0-5% margin range
      </div>
    );
  }
