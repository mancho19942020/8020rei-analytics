 ...
 *   SLACK_DM_ALERTS_CHANNEL_ID — Channel ID for #dm-campaign-alerts
 *
...
/**
 * Post a message to #dm-campaign-alerts. Returns the message `ts` (for threading).
 */
export async function sendSlackMessage(payload: SlackMessage): Promise<string | null> {
  if (BOT_TOKEN && DM_ALERTS_CHANNEL) {
    return postViaWebApi(payload);
  }
  ...
  console.log('[Slack] Not configured — missing SLACK_BOT_TOKEN + SLACK_DM_ALERTS_CHANNEL_ID or SLACK_DM_ALERTS_WEBHOOK_URL');
  return null;
}
...
export async function sendSlackThreadReply(
  ...
  if (!BOT_TOKEN || !DM_ALERTS_CHANNEL) {
    console.log('[Slack] Threading requires SLACK_BOT_TOKEN + SLACK_DM_ALERTS_CHANNEL_ID');
    return null;
  }

  return postViaWebApi(payload, threadTs);
}
