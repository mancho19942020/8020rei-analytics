...
export const DEFAULT_DM_OVERVIEW_LAYOUT: Widget[] = [
  {
    ...
    tooltip:
      'Executive view of DM Campaign health. Active clients is Aurora-only (rr_campaign_snapshots, distinct domains with ≥1 status=active campaign). Total delivered = dm_client_funnel.total_delivered (USPS-confirmed). Sub-line shows PCM shipped + the gap (pieces PCM sent that haven\'t been confirmed delivered — in-transit, returned, or undeliverable). Lifetime revenue is Aurora-only (PCM does not track customer revenue). Active campaigns is Aurora-only.',
    x: 0, y: 0,
    w: 12, h: 3,
...
export const DEFAULT_RAPID_RESPONSE_LAYOUT: Widget[] = [
  ...
  // Row 3: Q2 volume goal | Top contributors  (side-by-side)
  {
    ...
    tooltip: 'Progress toward 400K DM pieces dispatched for Q2 2026 (April-June). Goal definition: send 400K letters into the pipeline. Hero counts dispatched volume (PCM /order); Delivered (Q2) pill shows the subset USPS confirmed in mailboxes; In transit shows pieces dispatched but not yet landed.',
    x: 0, y: 5,
    w: 6, h: 5,
  ...
  },
  {
    ...
    tooltip: 'Per-client breakdown of Q2 sends. "Pieces sent" sums across all rows to the Q2 volume goal hero on the left (same source: PCM /order in Q2). "Pieces delivered" is the subset USPS confirmed in mailboxes. Share = client\'s % of total Q2 dispatched volume.',
    x: 6, y: 5,
    w: 6, h: 5,
...
export const RAPID_RESPONSE_WIDGET_CATALOG: WidgetCatalogItem[] = [
  {
    type: 'rr-q2-goal',
    title: 'Q2 volume goal',
    description: 'Progress bar + dispatched/delivered breakdown for the 400K Q2 target',
    iconKey: 'grid',
    defaultSize: { w: 4, h: 6 },
  },
  {
    type: 'rr-q2-top-contributors',
    title: 'Top contributors',
    description: 'Per-client deliveries in Q2 with share-of-total',
    iconKey: 'table',
    defaultSize: { w: 8, h: 6 },
  },
...
export const DEFAULT_DM_BUSINESS_RESULTS_LAYOUT: Widget[] = [
  // Row 1: Conversion Funnel — THE hero metric, answers "how is DM performing?"
  {
    ...
    tooltip: 'Strict-attribution view: counts every property whose log_status_properties row at each stage carries the DM campaign\'s rapid_response_id. Mirrors the platform\'s per-campaign detail page exactly (no date arithmetic — the platform doesn\'t require lead to follow send, and neither do we). Status changes done outside the DM workflow (manual edits, inbound calls, bulk imports) are unattributed and excluded. "Total delivered" stage = unique addresses where at least one piece was confirmed delivered (USPS-scanned). Cohort respects the date filter at the top of the page. Operational Health → Campaigns counts individual pieces, not properties, so numbers will differ by design.',
    x: 0, y: 0,
    w: 12, h: 7,
    minW: 6, minH: 4, maxW: 12, maxH: 10,
  ...
  // Row 2: Client Performance (full width, taller for more rows)
  {
    ...
    tooltip: 'Per-client breakdown filtered by the selected date range. "Mailed" = unique properties first mailed in the period. Leads/Deals use strict attribution and mirror the platform\'s per-campaign view exactly (only conversions whose log_status_properties row carries the DM campaign\'s rapid_response_id, no date gating). Click any number to see the actual properties.',
    x: 0, y: 7,
    w: 12, h: 8,
    minW: 6, minH: 5, maxW: 12, maxH: 10,
    timeBehavior: 'date-filtered',
  ...
  // Row 3: Template Leaderboard — optimization detail, not first-glance
  {
    ...
    tooltip: 'Templates ranked by performance within the selected date range. Sent/Delivered = mail pieces (matches OH). Leads/Deals use strict attribution: distinct properties whose log_status_properties row at each stage carries the campaign\'s rapid_response_id, mirroring the platform\'s per-campaign view exactly (no date gating). ROAS = client deal revenue ÷ client mail spend (client ROI, not 8020REI company margin).',
    x: 0, y: 15,
    w: 12, h: 6,
    minW: 6, minH: 5, maxW: 12, maxH: 10,
    timeBehavior: 'date-filtered',
