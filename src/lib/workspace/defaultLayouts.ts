...
export const DEFAULT_DM_BUSINESS_RESULTS_LAYOUT: Widget[] = [
  // Row 1: Conversion Funnel — THE hero metric, answers "how is DM performing?"
  {
    ...
    tooltip: 'Strict-attribution view: counts only conversions whose log_status_properties row carries the DM campaign\'s rapid_response_id — the same definition the platform\'s per-campaign detail page uses. Status changes done outside the DM workflow (manual edits, inbound calls, bulk imports) are unattributed and excluded. Cohort = properties first mailed in the selected window. "Mailed" = unique properties (addresses), not individual mail pieces (Operational Health counts pieces, so numbers differ by design).',
    x: 0, y: 0,
    w: 12, h: 7,
    minW: 6, minH: 4, maxW: 12, maxH: 10,
  ...
  // Row 2: Client Performance (full width, taller for more rows)
  {
    ...
    tooltip: 'Per-client breakdown filtered by the selected date range. "Mailed" = unique properties first mailed in the period. Leads/Deals use strict attribution (only conversions tagged with the DM campaign\'s rapid_response_id; matches platform per-campaign view). Click any number to see the actual properties.',
    x: 0, y: 7,
    w: 12, h: 8,
    minW: 6, minH: 5, maxW: 12, maxH: 10,
    timeBehavior: 'date-filtered',
  ...
  // Row 3: Template Leaderboard — optimization detail, not first-glance
  {
    ...
    tooltip: 'Templates ranked by performance within the selected date range. Sent/Delivered = mail pieces (matches OH). Leads/Deals use strict attribution: only conversions whose log_status_properties row carries the campaign\'s rapid_response_id (matches platform per-campaign view). ROAS = client deal revenue ÷ client mail spend (client ROI, not 8020REI company margin).',
    x: 0, y: 15,
    w: 12, h: 6,
    minW: 6, minH: 5, maxW: 12, maxH: 10,
    timeBehavior: 'date-filtered',
